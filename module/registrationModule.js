    ...
    addRegistration: async function (userID, eventID, totalMembers, teamName, teamMembers, memberRoles) {
        ...
        try {
            ...
            if(pendingPayments.length > 0){
                return setResponseBadRequest("You have already made an attempt to Register for an event that is still in pending state.");
            }
            ...
            const eventExistsResponse = await checkEventExistence(eventID, totalMembers, db);
            ...
            if(eventData[0].isGroup === 0){
                paymentAmount = eventData[0].eventFee + Math.ceil(eventData[0].eventFee * 0.18);
                ...
                const checkDuplicateTransactionResponse = await checkDuplicateTransaction(txnID, transactionDB);
                ...
                const [insertTransactionRecord] = await transactionDB.query(
                    ...
                    [txnID, userID, eventID, paymentAmount, userName, userEmail, phoneNumber, productInfo, "1"]
                );

                if(insertTransactionRecord.affectedRows !== 1) {
                    console.log("[ERROR]: Failed to Insert Record into transactionData");
                    const data = [txnID, userID, eventID, paymentAmount, userName, userEmail, phoneNumber, productInfo];
                    console.log(data);
                    appendFileSync('./logs/failedRegistrations.log', `${new Date().getTime}-"transactionData Table Insertion Failed"-${data}`);
                    throw new Error("Failed to Insert Record into transactionData Table");
                }
                ...
                const [insertRegistrationRecord] = await db.query(
                    ...
                    [eventID, userID, txnID, paymentAmount, totalMembers, teamName]
                );

                if(insertRegistrationRecord.affectedRows !== 1) {
                    console.log("[ERROR]: Failed to Insert Record into registrationData");
                    const data = [txnID, userID, eventID, paymentAmount, userName, userEmail, phoneNumber, productInfo];
                    console.log(data);
                    appendFileSync('./logs/failedRegistrations.log', `${new Date().getTime}-"registrationData Table Insertion Failed"-${data}`);
                    throw new Error("Failed to Insert Record into registrationData Table");
                }
                ...
                if(eventDataUpdate.affectedRows !== 1){
                    console.log("[ERROR]: Error in Inserting Udpating Seats in eventsData after Registration");
                    const data = [eventData[0].numRegistrations + totalMembers, eventID];
                    console.log(data);
                    appendFileSync('./logs/failedRegistrations.log', `${new Date().getTime}-"eventData Seats Update Failed"-${data}`);
            ...
            // Group Event.
                ...
                if(eventData[0].isPerHeadFee === 1) {
                    paymentAmount = (eventData[0].eventFee * totalMembers) + Math.ceil((eventData[0].eventFee * totalMembers) * 0.18);
                }
                ...
                const validGroupData = validateEventGroup(userEmail, teamMembers, memberRoles, totalMembers);
                ...
                if(userTeamDataCheck.length !== teamMembers.length) {
                    return setResponseBadRequest("Failed to Register. Invalid Email, one of the Team Member Has not Registered for Pragati 2025.");
                }
                ...
                if(insertRegistrationRecord.affectedRows !== 1) {
                    ...
                    const data = [txnID, userID, eventID, paymentAmount, userName, userEmail, phoneNumber, productInfo];
                    console.log(data);
                    appendFileSync('./logs/failedRegistrations.log', `${data}-${new Date().getTime}-"registrationData Table Insertion Failed"-${data}`);
                    throw new Error("Failed to Insert Record into registrationData Table");
                }
                ...
                const [insertLeaderGroupData] = await db.query(
                    ...
                    [insertRegistrationRecord.insertId, userID, eventID, "TEAM LEAD"]
                );

                if(insertLeaderGroupData.affectedRows !== 1) {
                    console.log("[ERROR]: Failed to Insert Record into groupDetail");
                    const data = [insertRegistrationRecord.insertId, userID, eventID, "TEAM LEAD"];
                    console.log(data);
                    appendFileSync('./logs/failedRegistrations.log', `${new Date().getTime}-"registrationData Table Insertion Failed"-${data}`);
                    throw new Error("Failed to Insert Record into groupDetail Table");
                }
                ...
                for(let i = 0; i < teamMembers.length; i++){
                    const [insertTeamGroupData] = await db.query(
                        "INSERT INTO groupDetail (registrationID, userID, eventID, roleDescription) VALUES (?, ?, ?, ?)",
                        [insertRegistrationRecord.insertId, userIDs[i], eventID, memberRoles[i]]
                    );
                ...
                if(eventDataUpdate.affectedRows !== 1){
                    console.log("[ERROR]: Error in Inserting Updating Seats in eventData after Registration");
                    const data = [eventData[0].numRegistrations + totalMembers, eventID];
                    console.log(data);
                    appendFileSync('./logs/failedRegistrations.log', `${data}-${new Date().getTime}-"eventData Seats Update Failed"-${data}`);
                    throw new Error("eventData Seats Update Failed");
                }
            ...
            appendFileSync('./logs/controller/controllerErrors.log', `${new Date().getTime}-"Registration Module Failed"-${err}`);
