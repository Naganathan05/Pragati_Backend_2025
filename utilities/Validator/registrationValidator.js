...
const validateEventRegistration = (userID, eventID, totalMembers, teamName) => {
    if (!(typeof (eventID) === "number" && eventID >= 1 &&
        ...
        typeof (teamName) === "string" && (teamName.length > 0 && teamName.length < 255))) {
        return false;
    }
    return true;
};

const validateEventGroup = (userEmail, teamMembers, memberRoles, totalMembers) => {
  if(!(typeof(teamMembers) === "object" && teamMembers.length === totalMembers - 1 && Array.isArray(teamMembers))){
    return "Failed to Register. Team Data Invalid";
  } 

  if(!(typeof(memberRoles) === "object" && memberRoles.length === totalMembers - 1 && Array.isArray(memberRoles))){
  ...
  for(let i = 0; i <= teamMembers.length - 1; i++) {
    ...
    if(typeof(memberRoles[i]) !== "string" || memberRoles[i].length === 0 || memberRoles[i].length >= 255) {
